...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        # * Fill in start (1)
        '''we had put here our ip address and a port that is private dynamic'''
        '''we had bind this socket to this address'''
        ...
        while True:
            try:
                ...
                # * Fill in start (2)
                '''here we accept from the client'''
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            # * Fill in start (3)
            '''we will not pass this code,
            until we reacied a mssage from the client
            and then put it in here as how many byte there is'''
            data = client_socket.recv(8180)
            # * Fill in end (3)
            if not data:
                break
            try:
                ...
                # * Fill in start (4)
                '''here we respond, and close connection and then break out the loop'''
